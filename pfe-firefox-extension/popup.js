...
function loadData() {
    // Use browser.storage API to get the stored data
    browser.storage.local.get(['stable_id', 'validation_code']).then(
        (result) => {
            if (result.stable_id) {
...
document.getElementById('saveButton').addEventListener('click', () => {
    ...
    // Save data using browser.storage API
    browser.storage.local.set({ stable_id: stable_id, validation_code: validation_code }).then(
